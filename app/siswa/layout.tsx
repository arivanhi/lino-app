...
export default function SiswaLayout({ children }: { children: React.ReactNode }) {
	...
	const menuItems = [
		{ name: "Dashboard", icon: LayoutDashboard, path: "/siswa/dashboard" },
		{ name: "Literasi", icon: BookOpen, path: "/siswa/literasi" },
		{ name: "Numerasi", icon: Calculator, path: "/siswa/numerasi" },
		{ name: "Riwayat Lino", icon: History, path: "/siswa/riwayat" },
	];
		...
		<div className="min-h-screen bg-slate-50 flex">
			...
			>
				<div className="p-6 border-b border-slate-100 flex items-center gap-3">
					...
					<div>
						<h2 className="text-lg font-bold text-slate-800 leading-tight">LiNO Portal</h2>
						<p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Academic Portal</p>
					</div>
				...
				<header className="h-16 bg-white border-b border-slate-200 px-4 sm:px-8 flex items-center justify-between sticky top-0 z-30">
					...
					<div className="flex items-center gap-3 bg-slate-50 px-3 py-1.5 rounded-full border border-slate-100">
						<div className="text-right hidden sm:block">
							...
							<p className="text-xs text-slate-500 font-medium mt-1 uppercase">{session?.user?.role}</p>
						</div>
